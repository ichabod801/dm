...
class CreatureError(ValueError):
	"""An error getting a creature. (ValueError)"""
	pass
...
class Egor(cmd.Cmd):
	...
	auto_attack: A flag for NPCs automatically attacking. (bool)
	auto_kill: A flag for automatically killing NPCs with 0 HP. (bool)
	...
	climate: The type of climate used for weather generation. (str)
	combatants: The creatures in the current combat. (dict of str: Creature)
	dex_tiebreak: A flag for using Dexterity to break initiative ties. (bool)
	encounters: Preset encounters. (dict of str: tuple)
	group_hp: A flag for giving all members of a group the same hp. (bool)
	...
	random_tiebreak: A flag for breaking initiative ties randomly. (bool)
	round: The number of the current combat round. (int)
	season: The current season used for weather generation. (str)
	srd: The stored Source Resource Document for D&D. (SRD)
	tables: Rollable tables loaded from markdown files. (dict of str: Table)
	tags: Tags for notes stored by the DM. (dict of str: list)
		For each tag, this dict has a list of indices to self.notes.
	time: The current game time. (gtime.Time)
	timer: A time checkpoint for autosaves. (float)
	voice: The text for communicating witht the user. (dict of str: str)
	weather_roll: The roll specification for generating weather. (str)
	xp: The xp earned by the party since the last distribution. (int)
	xp_method: The method for calculating experience points. (str)
	...
	do_autotag: Create automatic tags for every note. (None)
	...
	do_git: Egor doesn't understand git. (None)
	...
	do_show: Show the initiative order. (None)
	...
	init_sorter: Sort function for character initiative. (tuple)
	...
	set_changes: Set changes and update the auto-save timer. (None)
